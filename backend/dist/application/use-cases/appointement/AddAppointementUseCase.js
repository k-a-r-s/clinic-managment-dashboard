...
class AddAppointementUseCase {
    constructor(appointementRepository, medicalFileRepository, roomRepository) {
    ...
    async execute(addAppointmentDto) {
        ...
        const result = await this.appointementRepository.addAppointement(appointement);
        // Update medical file with appointment history
        try {
            const medicalFile = await this.medicalFileRepository.getMedicalFileByPatientId(addAppointmentDto.patientId);
            if (medicalFile) {
