...
class AppointementController {
    constructor(addAppointementUseCase, getAppointementsUseCase, getAppointementsByDoctorUseCase, getAppointementsByPatientUseCase, deleteAppointementUseCase, getAppointmentByIdUseCase, updateAppointmentUseCase, getAppointmentHistoryUseCase, getHistoriesByPatientUseCase, updateAppointmentHistoryUseCase, deleteAppointmentHistoryUseCase, appointmentCompletedUseCase) {
    ...
    async getAllAppointments(request, response) {
        const { view = "all", patientName, doctorName } = request.query;
        const filters = {
    ...
    async completeAppointment(request, response) {
        try {
            ...
            if (!patientId || !doctorId) {
                ResponseFormatter_1.ResponseFormatter.error(response, {
                    type: "VALIDATION_ERROR",
                    message: "patientId and doctorId are required",
                }, 400, "patientId and doctorId are required in request body");
                return;
            }
    ...
    async updateAppointment(request, response) {
        ...
        await this.updateAppointmentUseCase.execute(appointmentId, updateData);
        return ResponseFormatter_1.ResponseFormatter.success(response, null, "Appointment updated successfully");
    }
}
