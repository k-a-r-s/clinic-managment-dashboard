...
class AuthController {
    constructor(userAuthService, changePasswordUseCase, updateCurrentUserUseCase) {
    ...
    async updateProfile(req, res) {
        if (!this.updateCurrentUserUseCase) {
            ...
            }, 500, "Update profile not available");
        }
        const userId = req.user?.id;
        if (!userId) {
            return ResponseFormatter_1.ResponseFormatter.error(res, { type: "AuthenticationError", message: "User not authenticated" }, 401, "User not authenticated");
        }
        const updatedUser = await this.updateCurrentUserUseCase.execute(userId, req.body);
        return ResponseFormatter_1.ResponseFormatter.success(res, updatedUser, "Profile updated successfully");
    }
    async changePassword(req, res) {
        if (!this.changePasswordUseCase) {
            ...
            }, 500, "Change password not available");
        }
        const userId = req.user?.id;
        if (!userId) {
            return ResponseFormatter_1.ResponseFormatter.error(res, { type: "AuthenticationError", message: "User not authenticated" }, 401, "User not authenticated");
        ...
        return ResponseFormatter_1.ResponseFormatter.success(res, null, "Password changed successfully");
    }
}
