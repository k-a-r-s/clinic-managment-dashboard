...
CREATE TABLE public.machines (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  machine_id character varying NOT NULL DEFAULT ('HD-MAC-'::text || lpad((nextval('machines_machine_seq'::regclass))::text, 3, '0'::text)) UNIQUE,
  manufacturer character varying,
  model character varying,
  status USER-DEFINED NOT NULL DEFAULT 'available'::machine_status,
...
CREATE TABLE public.patients (
  ...
  CONSTRAINT patients_medical_file_id_fkey FOREIGN KEY (medical_file_id) REFERENCES public.patient_medical_files(id)
);
