...
export class AppointementController {
  ...
  async getAllAppointments(request: AuthRequest, response: Response) {
    ...
    const appointments = await this.getAppointementsUseCase.execute(
      view as "year" | "month" | "week" | "day" | "all",
      filters
    );
    return ResponseFormatter.success(
      response,
  ...
  async getAppointmentById(request: AuthRequest, response: Response) {
    ...
    if (!appointment) {
      return ResponseFormatter.error(
        response,
        { type: "NOT_FOUND", message: "Appointment not found" },
        404,
  ...
  async getAppointmentsByDoctor(request: AuthRequest, response: Response) {
    ...
    const appointments = await this.getAppointementsByDoctorUseCase.execute(
      doctorId,
      view as "year" | "month" | "week" | "day"
    );
    return ResponseFormatter.success(
      response,
      appointments,
  ...
  async getAppointmentsByPatient(request: AuthRequest, response: Response) {
    ...
    const { view = "month" } = request.query;
    const appointments = await this.getAppointementsByPatientUseCase.execute(
      patientId,
      view as "year" | "month" | "week" | "day"
    );
    return ResponseFormatter.success(
  ...
  async getHistoriesByPatientId(request: AuthRequest, response: Response) {
    try {
      const { patientId } = request.params;
      const histories = await this.getHistoriesByPatientUseCase.execute(
        patientId
      );
  ...
  async completeAppointment(request: AuthRequest, response: Response) {
    try {
      ...
      await this.appointmentCompletedUseCase.execute(
        medicalFileData,
        appointmentId
      );
  ...
  async updateAppointment(request: AuthRequest, response: Response) {
    const { appointmentId } = request.params;
