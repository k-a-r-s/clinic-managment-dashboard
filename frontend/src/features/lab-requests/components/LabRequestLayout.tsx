...
const LabRequestLayout: React.FC = () => {
  ...
  useEffect(() => {
    setHasUnsavedChanges(
      selectedTests.length > 0 || otherTests.trim().length > 0
    );
    ...
    if (draft) {
      try {
        ...
        const shouldLoad = window.confirm(
          "A draft lab request was found. Would you like to load it?"
        );
        if (shouldLoad) {
    ...
    <div className="p-6 space-y-6 bg-gray-50">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          />
          <OtherTestsSection value={otherTests} onChange={setOtherTests} />
        </div>

        {/* Right Section - Print Preview */}
        <div className="lg:col-span-1">
          <PrintPreview selectedTests={selectedTests} otherTests={otherTests} />
        </div>
      </div>
    </div>
  );
